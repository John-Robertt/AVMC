...
本文件描述 AI 工具在本项目中的工作结构、信息来源、任务路由和协作流程。

项目中的信息结构分为 3 层：

- `CLAUDE.md`：AI 工作方式与任务路由
- `docs/`：项目规则、接口契约与文档索引
- 代码与测试：当前实现的直接证据
...
1. 从 `docs/README.md` 进入当前主题的权威文档。
2. 读取对应代码与测试，用函数名、类型名和符号引用建立定位。
3. 在与当前目标直接相关的范围内实施修改。
4. 运行验证命令：`go test ./...` 与 `go vet ./...`。
5. 对接口、行为语义、约束条件、输出结构或用户可见提示产生影响时，同步更新对应权威文档。

## 文档表达

- 正文聚焦当前存在的能力、结构、流程、接口、用法和设计意图。
- 正文组织方式以“是什么、怎么用、怎么工作”为主。
...
- `docs/README.md`：文档注册表与权威源入口
- 现行规范：描述当前接口、行为、结构和契约
- `docs/DEVELOPMENT_PLAN.md`：工作计划容器
- `docs/adr/`：决策背景与架构记录

## 当前协作方式

- 以文档建立上下文，以代码和测试确认实现细节。
- 以最小必要范围组织修改。
- 以验证结果收束变更。
- 以权威文档同步维护项目知识。
