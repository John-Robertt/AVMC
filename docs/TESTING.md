...
| 位置 | 当前覆盖内容 |
| --- | --- |
| `internal/config/config_test.go` | 配置文件发现、`path/provider/apply` 覆盖顺序、`image_proxy` 与 `proxy.url` 约束、非法 provider / proxy URL |
...
| `internal/domain/report_test.go` | `RunReport.Finalize` 的 UTC、排序和 summary |
| `internal/app/run/e2e_test.go` | dry-run 零写入；apply 生成 sidecar、写 cache、移动视频 |
