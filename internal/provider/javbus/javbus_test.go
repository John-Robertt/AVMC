...
func TestParse_Golden(t *testing.T) {
	...
	sort.Strings(names)
	if len(names) == 0 {
		t.Fatalf("未找到任何 fixture（testdata/*.html）")
	}
	...
	for _, name := range names {
		...
		if pageURL == "" {
			pageURL = "https://www.javbus.com/" + base
		}
		...
		if string(want) != string(got) {
			t.Fatalf("golden 不匹配：%s（重新生成：UPDATE_GOLDEN=1 go test ./internal/provider/javbus）", goldenPath)
		}
	}
}
